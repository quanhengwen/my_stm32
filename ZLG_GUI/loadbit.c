...
{  
   ...
   sX = x;
		...
		{
	    sY = y;
			for (i = 0; i < lno; i++)	/* 字符高度 */
			{
				if ((dat[(m  + (i/8)*hno)] & (0x01 << (i % 8 ))) != 0x00)
				{
					GUI_CopyColor(&bakc, disp_color);	/* 设置像素颜色为文字色 */
				}
				else
				{
					GUI_CopyColor(&bakc, back_color);	/* 设置像素颜色为文字背景色 */
				}
					GUI_Point(sX, sY, bakc); 
				sY++;
			}
...
void GUI_PutHZ(uint32 x, uint32 y, uint8 *dat,uint8 fontsize)
{
	Show_Str1(x,y,dat,fontsize);
}
#endif
